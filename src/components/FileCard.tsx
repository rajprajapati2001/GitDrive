...
export function getFileIcon(name: string, type: "file" | "dir") {
  ...
  switch (ext) {
    ...
    case "txt":
    case "md":
      return <FileText className="w-10 h-10 text-orange-500 fill-orange-500/5" />;
    default:
...
export default function FileCard({
  ...
  return (
    <div
      onClick={onClick}
      className={`group relative bg-white border border-slate-250/60 hover:border-orange-500 hover:shadow-md hover:shadow-orange-500/5 rounded-2xl p-4.5 select-none transition-all duration-200 cursor-pointer flex flex-col justify-between`}
    >
      ...
      <div>
        ...
        <h4
          title={item.name}
          className="text-sm font-semibold text-slate-800 break-words line-clamp-1 group-hover:text-orange-600 transition-colors"
        >
          {item.name}
        </h4>
      ...
      <div className="mt-4 pt-3 border-t border-slate-50 flex items-center justify-between text-[11px] text-slate-400 font-medium">
        ...
        <span className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity text-orange-500 font-semibold">
          {isDir ? (
            <>
